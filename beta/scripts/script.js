...
function createPicTable() {
    let tableHeaders = ['Качество', 'Яркость', 'Размытость','Задержка'];
...
function appendPicRow(data) {
    ...
    if (data.delay){
      delay.innerText = 'Нет задержки';
    }
    else{
      delay.innerText = 'Есть задержка';
    }
    ...
    newRow.append(quo, brit, blur,delay);
    table.append(newRow);
}
